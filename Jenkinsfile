...
jenkinsfileURL = "https://github.com/aslam-ahmed/apic-appc-cicd.git"
...
devServer = "apimanager.us-south.apiconnect.cloud.ibm.com"
devCatalog = "sb"
devOrg = "aslamahmedoutlookcom-dev"
...
def apic_initiate() {
    ...
    //withEnv(["PATH+EXTRA=/home/ec2-user/.nvm/versions/node/v10.22.1/bin"]){
...
def login(String server, String creds){
    ...
    withCredentials([string(credentialsId: 'apic_key', variable: 'secret')]) {
        ...
        sh "apic login -s ${server} -k ${secret}"        
    } 
...
def publish(String product, String catalog, String org, String server, String space = ""){
    ...
    def productPath = product + "/" + apiPath
    if (!space.trim()) {
